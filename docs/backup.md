...
## Backup to Archive
    ...
    docker run -v $OVPN_DATA:/etc/openvpn --rm damshenas/openvpn tar -cvf - -C /etc openvpn | xz > openvpn-backup.tar.xz
...
Creates an volume container named `$OVPN_DATA` to extract the data to.
    ...
    xzcat openvpn-backup.tar.xz | docker run -v $OVPN_DATA:/etc/openvpn -i damshenas/openvpn tar -xvf - -C /etc
