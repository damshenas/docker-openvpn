...
2. Initialize the data container, but don't start the container :
       ...
       docker run -v $OVPN_DATA:/etc/openvpn --rm damshenas/openvpn ovpn_genconfig -u udp://VPN.SERVERNAME.COM
       docker run -v $OVPN_DATA:/etc/openvpn --rm -it damshenas/openvpn ovpn_initpki
   
3. Download the [docker-openvpn@.service](https://raw.githubusercontent.com/damshenas/docker-openvpn/master/init/docker-openvpn%40.service)
   file to `/etc/systemd/system`:

        curl -L https://raw.githubusercontent.com/damshenas/docker-openvpn/master/init/docker-openvpn%40.service | sudo tee /etc/systemd/system/docker-openvpn@.service
